...
export const registerUser = async (data) => {
    try {
    await connectDB();
    const { name, email, password, phoneNumber, street, city, state, zip } = data;
    const user = await User.findOne({ email });
    ...
    const newUser = await User.create({ name, email, password: hashPassword,phoneNumber ,address: { street, city, state, zip } });
